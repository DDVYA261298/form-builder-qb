...
export function validate(choicesArray, maxCharacterLength) {
  ...
  if (choicesArray.length > MAX_CHOICES) {
    errors.choicesError = `* You have entered ${
      choicesArray.length
    } choices (maximum of ${MAX_CHOICES} allowed). Please delete ${
      choicesArray.length - MAX_CHOICES
    } before saving. Note: Default Choice should not be deleted.`;
  }
...
function findDuplicates(choicesArray) {
  ...
  const duplicates = choicesArrayLowerCase.reduce(function (
    acc,
    curr,
    index,
    srcArr
...
function findLongChoices(choicesArray, maxCharacterLength) {
  const longWords = choicesArray.filter(
    (choice) => choice.length > maxCharacterLength
  );
  return longWords;
}
